...
/*
based on 2sum, tried to do prone, but doesn't work

go back to general O(n^3) solution
...
public:
  vector<vector<int> > fourSum(vector<int>& nums, int target)  {
		...
		for (i_out=0;i_out<length-2;i_out++)
		{
			for (j_out=length-1;i_out<j_out-2;j_out--) 
			{
			...
			*/
			...
			//cout <<i_out<<" "<<j_out<<endl;
				...
				while (nums[j_out-1]==nums[j_out]) j_out--;
			//cout<<i_out<<" "<<j_out<<endl;
			
			}
			while (nums[i_out+1]==nums[i_out]) i_out++;
		}
